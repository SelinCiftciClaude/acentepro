    ...
    <div className="space-y-6">
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-1">
          <div className="text-sm font-medium text-gray-500">Toplam Komisyon Geliri</div>
        </div>
        <div className="text-3xl font-bold text-green-600">{fmtCurrency(stats.total_commission)}</div>
      ...
      <div className="card">
        ...
        <div className="divide-y divide-gray-50">
          ...
          {stats.recent_shipments?.map(s => (
            <Link key={s.id} to={`/sevkiyatlar/${s.id}`} className="flex items-center justify-between px-5 py-3.5 hover:bg-gray-50 transition-colors">
              <div>
                <div className="text-sm font-medium text-gray-900">{s.customer_name}</div>
                <div className="text-xs text-gray-400 mt-0.5">{s.company_name || '—'} · {fmtDate(s.shipment_date)}</div>
              </div>
              <div className="flex items-center gap-3">
                {s.commission_amount
                  ? <span className="text-sm font-semibold text-green-600">{fmtCurrency(s.commission_amount, s.currency)}</span>
                  : <span className="text-xs text-gray-400">Komisyon bekleniyor</span>
                }
